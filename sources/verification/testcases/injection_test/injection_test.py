...
async def injection_test(dut:SimHandleBase):
  ...
  await apb_master.busy_send(apb.APBTransaction(address = csr__address_range_start_0__address,     data = 0x1000 )) # Start address
  await apb_master.busy_send(apb.APBTransaction(address = csr__address_range_stop_0__address,      data = 0x4000 )) # Start address
  await apb_master.busy_send(apb.APBTransaction(address = csr__address_range_increment_0__address, data = 0x20   )) # Address increment
  ...
  await apb_master.busy_send(apb.APBTransaction(address = csr__transaction_attributes__address,    data = 0x1 | 0x1 << 1 )) # Write transaction
